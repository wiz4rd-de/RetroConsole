...
- **M6 — Hardware hardening (post-MVP)**: real-hardware matrix (Intel/AMD/NVIDIA), HDMI audio, Bluetooth pads, pixman/gamescope fallbacks, docs.
  *First real-hardware test (2026-06-12, keyboard attached):* live session works end to end (ES-DE, gamepad, bundled NES game). Four bugs found and fixed:
  1. **Installer: TV lost signal right after "Triggering uevents" and never recovered → install impossible.** Cause: the `kms` initramfs hook modesets to a mode/connector the TV won't display; the live session recovers only because cage later modesets the connected output itself (which is also why the shutdown tty was visible — fbcon inherits cage's mode). Fix: `nomodeset` on all installer boot entries (syslinux/systemd-boot/grub/loopback) — the installer is text-only and stays on the firmware framebuffer that already displayed the boot menu.
  2. **Installer (second attempt, after the nomodeset fix): "Cannot find /run/archiso/bootmnt/.../airootfs.sfs" right after autologin.** Cause: archiso's `copytoram=auto` copies the squashfs to RAM and *unmounts the boot medium* whenever the medium is non-optical, the image is <4 GiB, and RAM ≥ image+2 GiB — i.e. always on a real USB stick, never in our QEMU runs, which attached the ISO as a CD (`/dev/sr0` is exempt). Reproduced in QEMU with the new `scripts/test-qemu.sh --usb-stick` flag (attaches the ISO as USB mass storage). Fix: `copytoram=n` on the installer entries (the installer reads squashfs + kernel from the mounted medium); live entries keep the RAM copy on purpose (faster, stick can be pulled). The installer now also detects the copytoram case and explains it instead of the misleading "still attached?" message.
  3. **Audio came from the PC's internal speaker, not the TV.** Fix: WirePlumber drop-in (`50-retroconsole-prefer-hdmi.conf`) raising HDMI sink priority above analog. *Caveat:* only helps when an HDMI sink node exists; if analog + HDMI share one ALSA card and WirePlumber picks the analog-only profile, profile selection needs forcing too — needs `wpctl status` output from the machine if sound is still wrong.
  4. **Wi-Fi setup fails with "missing privileges" (reported from the field, reproduced in QEMU).** Saving a Wi-Fi network in nmtui creates a NetworkManager *system connection*, which polkit gates behind `auth_admin` — and the kiosk session has no polkit authentication agent, so it's denied outright (`nmcli con add` → "Insufficient privileges"). The M4 QEMU check only opened and closed nmtui, which needs no privilege. Fix: `/etc/polkit-1/rules.d/50-retroconsole-network.rules` granting `retro` all `org.freedesktop.NetworkManager.*` actions, plus `org.bluez.*` preemptively (device pairing sits behind the same wall). Verified in QEMU: the same `nmcli con add` succeeds on the fixed image.
  *Known cosmetic:* live boot shows ~30 s of "no signal" between the kms handover and the ES-DE splash — expected, document rather than fix (a quiet/plymouth-style splash would need the same modeset anyway).
- **M7 — OTA updates** (user request after repeated reflash-wipe cycles): system-level appliance files moved out of airootfs into the `retroconsole-config` package (`packages/retroconsole-config/`), built into the `[retroconsole]` repo on every `make iso`; repo published to a rolling GitHub Release (`scripts/publish-repo.sh`); image `pacman.conf` lists the GitHub URL first with the baked-in `file://` repo as offline fallback. Sudoers split: packaged `retroconsole` (update/power rights) vs airootfs-only `retroconsole-live` (installer right, removed by the installer — replaces the old sed). `/home/retro` seeds deliberately stay airootfs (runtime-mutable user state). *Bootstrap caveat:* systems installed before M7 have a file-only pacman.conf and no root path (root locked, retro's sudo limited) — they need one GRUB-edit (`systemd.debug-shell=1`, root shell on tty9, fix pacman.conf + `pacman -Syu`) or one final reinstall.
- **M8 — release CI** (`.github/workflows/release.yml`): pushing a semver tag (`v0.1.0`+) builds the ISO on a native amd64 runner and, with no manual uploads, creates the versioned GitHub Release (ISO + `.sha256`) *and* republishes the rolling `repo` OTA channel (`scripts/publish-repo.sh`). Tag-driven and OTA-on-release-only by decision: the full ISO build and the OTA refresh move together. Uses the built-in `GITHUB_TOKEN` (`contents: write`); AUR packages compile fresh each release (no cross-run cache → reproducible artifacts).
  *Backlog:* gamepad-only Wi-Fi setup (current `nmtui` tool needs a keyboard; Batocera's ES fork has a built-in network UI, ES-DE does not — candidate: a controller-driven picker, e.g. gptokeyb-style pad→keyboard mapping over a dialog/nmtui wrapper); Finder/avahi discovery; Bluetooth pairing flow; untested pad models.
